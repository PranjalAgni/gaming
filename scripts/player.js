...
export class Player {
  ...
  onKeyDown(event) {
    ...
    switch (event.code) {
      ...
      case "F10":
        ...
        break;
      case "KeyX":
        // Toggle the crafting panel instead of jumping
        this.toggleCraftingContainer();
        break;
    }
  }
  ...
  toggleCraftingContainer() {
    const craftDiv = document.getElementById("crafting-container");
    if (!this.craftingOpen) {
      // Show the container
      craftDiv.style.display = "block";
      // Optionally unlock controls so mouse can click inside UI
      this.controls.unlock();
    } else {
      // Hide the container
      craftDiv.style.display = "none";
      // Re-lock pointer controls if desired
      this.controls.lock();
    }
